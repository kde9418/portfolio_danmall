  ...
  immediately after the control sidebar -->
	...
	<script>
		$(document).ready(function(){
			...
			// "pdt_detail" : text-area를 가르켜주는 id
			...
			$("#mainCategory").on("change", function(){
				...
				var url = "/admin/product/subCategoryList/" + mainCategoryCode // url매핑주소를 경로형태로 사용 @PathVariable

				// ajax 기능을 가지고 있음
				// REST 방식으로 전송
				$.getJSON(url, function(subCGListData){ // subCGListData : 2차카테고리 데이터 정보

					// 2차카테고리 추가작업 -> 핸들바를 통해서
	...
	<%-- 2차 카테고리 템플릿 적용 함수 --%>
	<script>
		var subCategoryList = function(subCGListData, subCategoryTarget, subCategoryListTemplate){
			var subCGTemplate = Handlebars.compile(subCategoryListTemplate.html()); // subCategoryListTemplate 템플릿에 들어있던 내용을 읽어온다. Handlebars의 문법으로 compile 해서 객체 생성
			var options = subCGTemplate(subCGListData); // 템플릿에 2차카테고리 데이터가 바인딩된 소스

			$("#subCategory option").remove(); // 기존 option태그 제거(누적방지)
			subCategoryTarget.append(options); // 새로운 2차 카테고리 option 태그 추가
		}
	</script>
