...
<body class="hold-transition skin-blue sidebar-mini">
  ...
  immediately after the control sidebar -->
	...
	<script>
		...
		$(document).ready(function(){
			...
			$("button[name='btn_order_detail']").on("click", function(){
				...
				//ajax 호출작업 : url? 컨트롤러 매핑주소 생성하고, Postman 테스트

				var detailButton = $(this).parents("tr");
				var odr_code = $(this).attr("data-odr_code");
				var url = "/admin/order/order_detail/" + odr_code;

				// alert(url);
				// 1차카테고리를 참조하는  2차카테고리 정보
				$.getJSON(url, function(data){
					orderDetailList(data, detailButton, $("#orderDetailTemplate"));
				});
			});
	...
	<script>
		var orderDetailList = function(orderDetailData, orderCodeTarget, orderDetailTemplate){
			var detailTemplate = Handlebars.compile(orderDetailTemplate.html());
			var details = detailTemplate(orderDetailData);

			//<tr class="orderList">
			$(".orderDetail").remove(); // 상세출력된 태그 제거
	...
	<script id="orderDetailTemplate" type="text/x-handlebars-template">
		...
		<tr class="orderDetail">
			<th colspan="8" style="text-align: center;">주문상세내역</th>
		</tr>
		<tr class="orderDetail">
			<th style="text-align: center;">주문번호</th>
			<th style="text-align: center;">상품명</th>
			<th colspan="2"  style="text-align: center;">상품이미지</th>
		...
		{{#each .}}
		<tr class="orderDetail">
			<td style="text-align: center;">{{odr_code}}</td>
			<td style="text-align: center;">{{pdt_name}}</td>
			<td colspan="2" style="text-align: center;">
				<img src="/admin/order/displayFile?fileName={{pdt_img}}">
			</td>
			<td style="text-align: center;">{{odr_amount}}</td>
			<td style="text-align: center;">{{odr_price}}</td>
			<td colspan="2" style="text-align: center;">{{sum}}</td>
			
		</tr>
		{{/each}}
		<tr class="orderDetail">
			<td colspan="8" style="text-align: center;">합 계 : <b>주문상세내역</b></td>
		</tr>
	</script>
